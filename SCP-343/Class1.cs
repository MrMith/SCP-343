...
namespace SCP_343
{
    ...
    public class Main : Plugin
    {
        ...
        public override void Register()
        {
            ...
            this.AddEventHandler(typeof(IEventHandlerCheckRoundEnd), new MainLogic(this), Priority.Normal);
        }
    }
}
...
namespace SCP_343Logic
{
    public class MainLogic : IEventHandlerPlayerPickupItem,IEventHandlerRoundStart,IEventHandlerDoorAccess, IEventHandlerSetRole
    {
        ...
        public void OnPlayerPickupItem(PlayerPickupItemEvent ev)
        {
            if (ev.Player.TeamRole.Role == Role.TUTORIAL)
            {
                    plugin.Info(ev.Item + " wow that a item!");
        ...
        {
            plugin.Info(ev.Door.Name + " door one");
            //plugin.Info(TheChosenOne.Name + "THAT A NAME");
            if (ev.Player.TeamRole.Role == Role.TUTORIAL)
            {
                ev.Allow = true;
            }
